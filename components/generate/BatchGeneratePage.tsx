...
interface BatchGeneratePageProps { clients: Client[]; }
...
export function BatchGeneratePage({ clients }: BatchGeneratePageProps) {
  ...
  const handleBatchGenerate = useCallback(async () => {
    ...
    for (let i = 0; i < batchSize; i++) {
      try {
        ...
        if (!res.ok) { const data = await res.json(); newResults.push({ url: "", error: data.error || "Failed" }); }
        else { const data = await res.json(); newResults.push({ url: data.outputUrl }); }
      } catch (err) { newResults.push({ url: "", error: err instanceof Error ? err.message : "Failed" }); }
      setProgress(i + 1);
  ...
  return (
    <div className="generate-page">
      ...
      {error && (<div className="generate-error"><span>{error}</span><button onClick={() => setError(null)}>\u00d7</button></div>)}

      <div className="generate-content">
        <div className="generate-panel">
          {selectedImage ? (
            <div className="selected-preview"><label>Reference Image</label><img src={selectedImage.url} alt="Selected reference" /></div>
          ) : (
            <div className="selected-preview empty"><span>Select an image from the library</span></div>
          )}

          <div className="generate-setting"><label>Batch Size</label><div className="setting-options">
            {BATCH_SIZES.map((size) => (<button key={size} className={`setting-option ${batchSize === size ? "active" : ""}`} onClick={() => setBatchSize(size)} type="button">{size}</button>))}
          </div></div>

          <div className="generate-setting"><label>Aspect Ratio</label><div className="setting-options">
            {ASPECT_RATIOS.map((ar) => (<button key={ar.value} className={`setting-option ${aspectRatio === ar.value ? "active" : ""}`} onClick={() => setAspectRatio(ar.value)} type="button">{ar.label}</button>))}
          </div></div>

          <div className="generate-setting"><label>Resolution</label><div className="setting-options">
            {RESOLUTIONS.map((r) => (<button key={r.value} className={`setting-option ${resolution === r.value ? "active" : ""}`} onClick={() => setResolution(r.value)} type="button">{r.label}</button>))}
          </div></div>

          <div className="generate-prompt"><label htmlFor="prompt-input">Prompt</label>
          ...
          {generating && (<div className="batch-progress"><div className="batch-progress-bar" style={{ width: `${(progress / batchSize) * 100}%` }} /></div>)}

          {results.length > 0 && !generating && (<div className="batch-summary"><span>{successCount} succeeded</span>{failCount > 0 && <span className="batch-fail">{failCount} failed</span>}</div>)}
        </div>
              ...
              <div className="batch-results-grid">
                {results.filter((r) => r.url).map((r, i) => (
                  <div key={i} className="batch-result-item">
                    <img src={r.url} alt={`Generated ${i + 1}`} onClick={() => setPreviewUrl(r.url)} className="clickable-image" />
                    <div className="batch-result-actions">
                      <button className="btn-icon" onClick={() => handleDownload(r.url, `batch-${i + 1}.png`)} title="Download">⬇</button>
                      <button className="btn-icon" onClick={() => setPreviewUrl(r.url)} title="Preview">🔍</button>
                    </div>
                  </div>
                ))}
              </div>
