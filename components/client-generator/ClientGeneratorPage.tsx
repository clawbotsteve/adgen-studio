    ...
    async (clientId: string) => {
      ...
      try {
        ...
        if (r.ok) {
          ...
          if (data && data.id) {
            setContext({ ...EMPTY_CONTEXT, ...data });
            // Load docs
            const dr = await fetch(
              `/api/brand-context/docs?brandContextId=${data.id}`
            );
            if (dr.ok) {
  ...
  const saveContext = async () => {
    ...
    try {
      const r = await fetch("/api/brand-context", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ clientId: selectedClientId, ...context }),
      });
      if (r.ok) {
        const data = await r.json();
        setContext((prev) => ({ ...prev, id: data.id }));
