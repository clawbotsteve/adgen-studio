...
export async function POST(request: Request) {
  ...
  const selectedAngles: string[] = Array.isArray(body.selectedAngles) ? body.selectedAngles : [];
  ...
  try {
    ...
    const savedPrompts: SavedPrompt[] = ((client.defaults as Record<string, unknown>)?.generatedPrompts as SavedPrompt[]) || [];
    ...
    if (savedPrompts.length > 0) {
      ...
      if (selectedAngles.length > 0) {
        if (lockAngles) {
          ...
          filtered = savedPrompts.filter(p => selectedAngles.includes(p.angle));
        } else {
          // Bias toward selected angles: put them first
          const selected = savedPrompts.filter(p => selectedAngles.includes(p.angle));
          const others = savedPrompts.filter(p => !selectedAngles.includes(p.angle));
      ...
      console.log("[smart-batch] Using " + prompts.length + " saved prompts from Client Generator");
    } else {
      // ── Fallback: generate prompts on the fly with Grok ──
      console.log("[smart-batch] No saved prompts found, falling back to Grok generation");
      ...
      if (!masterContext) {
        return NextResponse.json(
          { error: "No brand context or saved prompts found. Please complete the Client Generator first." },
          { status: 400 }
        );
      }
      ...
      try {
        generatedPrompts = await generatePromptsFromClient(masterContext, client.name, quantity);
      } catch (genErr) {
    ...
    if (prompts.length === 0) {
      return NextResponse.json(
        { error: "No prompts available. Generate prompts in the Client Generator first." },
        { status: 400 }
      );
    }
