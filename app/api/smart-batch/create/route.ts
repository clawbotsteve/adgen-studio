...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Get prompt items
    const allPromptItems = await listPromptItems(body.promptPackId);

    if (allPromptItems.length === 0) {
      return NextResponse.json(
    ...
    // Limit to requested quantity (default: use all items)
    const quantity = body.quantity && body.quantity > 0
      ? Math.min(body.quantity, allPromptItems.length)
      : allPromptItems.length;
    const promptItems = allPromptItems.slice(0, quantity);
